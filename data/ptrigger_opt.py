...
def optimize_trigger_config(data_path, labels_path, output_root="trigger_optimization.root", output_pdf="trigger_optimization_report.pdf"):
    ...
    for bin_thr in bin_thresholds_to_test:
        ...
        for thresh in thresholds:
            ...
            thr_vals.append(thresh_rms) # Plotting X-axis in RMS units
    ...
    limit_line = ROOT.TLine(0, MAX_TRIGGER_RATE, max_amp_overall_rms, MAX_TRIGGER_RATE)
    limit_line.SetLineColor(ROOT.kRed)
    limit_line.SetLineStyle(9)
    limit_line.SetLineWidth(2)
    limit_line.Draw("same")
    
    leg_rate = ROOT.TLegend(0.50, 0.65, 0.88, 0.88)
    ...
    for b in bin_thresholds_to_test:
        ...
        if safe_thresholds_rms[b] is not None:
            st_rms = safe_thresholds_rms[b]
            st_hw = safe_thresholds_hw[b]
            
            vl = ROOT.TLine(st_rms, 1e-5, st_rms, MAX_TRIGGER_RATE)
            ...
            # Shows RMS clearly, with Hardware value in parenthesis
            txt.DrawLatex(st_rms + 0.1, MAX_TRIGGER_RATE * 1.5, f"{st_rms:.2f} RMS (HW:{st_hw})")
