...
`CNN_THRESH` is a single global CNN trigger threshold, not a per-channel, per-lane, or per-class setting. It is a configuration input consumed in the `CLK_CNN` domain after a configuration CDC path. A threshold update takes effect on a CNN chunk boundary: the CNN-domain control path latches a threshold snapshot when starting a 256-sample chunk and uses that snapshot for the full chunk decision.
